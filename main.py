...
joke = ["Quel est le plat préféré de Thor? Thorresmo","Que fait le cheval dans la cabine téléphonique? Passer un coup de fil","Quel est le fleuve le plus acide du monde? Le Rio Solimões","Quel est l'endroit le plus droit du Brésil? Le sertão.","Quel est le vin qui n'a pas d'alcool? Le vin d'œuf de caille"]
...
class Bot(BaseBot):
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        print("funcionando")
        await self.highrise.walk_to(Position(13.5 , 0.0 , 10.0 , "FrontRight"))
    async def on_user_join(self, user: User, position: Position | AnchorPosition) -> None:
        print(f"{user.username} entrou na sala")
        await self.highrise.chat(f"Bienvenue dans❤️GRAB 10g❤️.Pour danser utilise de 1 à 95.Ecris help pour voir les commandes")
    ...
    async def on_chat(self, user: User, message: str) -> None:
        ...
        if message.lower() == "/combien de personne m'aiment?":
           frase = random.choice(amour)
           await self.highrise.send_whisper(user.id,frase)
        if message.lower() == "/combien de personne me détestent?":
        ...
        if        message.startswith("Help") or      message.startswith("/help") or      message.startswith("!help") or message.startswith("help"):
            await self.highrise.chat(f"/list  , /drague,  /emotes, /joke  , /emote-id , /couteau , /bombe , /curatif , /play , /tirer , /folie , /mariage?")
    ...
    async def on_user_leave(self, user: User) -> None:
        ...
        await self.highrise.chat(f" @{user.username} a grab ou pas?😂 Si non il/elle est radin")
        await self.highrise.send_emote("emote-launch")